...
public class TypeAnnotator: ITypeAnnotator
{
    ...
    // TODO: Complete this
    private const string UserFewShotSignaturesResponsse =
        """
    ...
    private const string AssistantFewShotFinal =
        """
        CALL_CHAIN
        1. GET all extension method specs :: extension-method-spec[]
           - Rule 'all-extension-method-specs-for' with targetType = "IServiceCollection"
        2. NATURAL LANGUAGE :: extension-method-spec[]
           - "Filter the subset from $1 such that the spec is handler-related."
        3. FOR EACH spec in $2, GET method declaration :: method[]
           - Rule 'extension-method-from-spec' with extendedType = "IServiceCollection", spec = element of $2
        """;

    public Task<string> AnnotateAsync(string nlStructure, CompositionContext context, CancellationToken cancellationToken = default)
    {
